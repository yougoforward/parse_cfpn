...
__all__ = ['fatnet2', 'get_fatnet2']
# add 1x1 channel transform for local patch
up_kwargs = {'mode': 'bilinear', 'align_corners': True}
class fatnet2(nn.Module):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d,  base_size=520, crop_size=480, mean=[.485, .456, .406],
        ...
        self.base = fatnet2_base(norm_layer)
        self.head = fatnet2Head(48, nclass, norm_layer, up_kwargs=self._up_kwargs)
...
class fatnet2_base(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d):
        super(fatnet2_base, self).__init__()
        self.layer1 = fatnet2_layer(3,64,1,1,norm_layer)
        self.layer2 = fatnet2_layer(64,64,1,1,norm_layer)
        
        self.layer3 = fatnet2_layer(64,48,1,2,norm_layer)
        self.layer4 = fatnet2_layer(48,48,1,2,norm_layer)
        
        self.layer5 = fatnet2_layer(48,48,1,4,norm_layer)
        self.layer6 = fatnet2_layer(48,48,1,4,norm_layer)
        self.layer7 = fatnet2_layer(48,48,1,4,norm_layer)
        
        self.layer8 = fatnet2_layer(48,48,1,8,norm_layer)
        self.layer9 = fatnet2_layer(48,48,1,8,norm_layer)
        self.layer10 = fatnet2_layer(48,48,1,8,norm_layer)
        
        self.layer11 = fatnet2_layer(48,48,1,16,norm_layer)
        self.layer12 = fatnet2_layer(48,48,1,16,norm_layer)
        self.layer13 = fatnet2_layer(48,48,1,16,norm_layer)
...
class fatnet2_layer(nn.Module):
    def __init__(self, in_planes, out_planes, dilation=1, tl_size=1, norm_layer=nn.BatchNorm2d):
        super(fatnet2_layer, self).__init__()
        self.tl_size = tl_size
        self.inplanes = in_planes
        self.outplanes = out_planes
        self.conv = nn.Sequential(nn.Conv2d(in_planes*tl_size*tl_size, out_planes//2*tl_size*tl_size, 3, padding=1, dilation=1, groups=tl_size*tl_size, bias=False),
                                   norm_layer(out_planes//2*tl_size*tl_size), nn.ReLU())
        self.conv2 = nn.Sequential(
                                   nn.Conv2d(out_planes*tl_size*tl_size//2, out_planes*tl_size*tl_size, 1, padding=0, dilation=1, groups=1, bias=False),
...
def get_fatnet2(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                 root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = fatnet2(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
